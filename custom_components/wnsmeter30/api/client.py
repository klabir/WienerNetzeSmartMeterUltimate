...
class Smartmeter:
    ...
    def _get_api_key(self, token):
        ...
        try:
            ...
            raise SmartmeterConnectionError("Could not parse API key response") from exception
        ...
        # Resolve API URLs from portal config for this client instance only.
        b2c_api_url = result.get("b2cApiUrl")
        if b2c_api_url and b2c_api_url != self._api_url:
            self._api_url = b2c_api_url
            logger.warning(
                "The b2cApiUrl has changed to %s! Using instance-local URL.",
                self._api_url,
            )
        b2b_api_url = result.get("b2bApiUrl")
        if b2b_api_url and b2b_api_url != self._api_url_b2b:
            self._api_url_b2b = b2b_api_url
            logger.warning(
                "The b2bApiUrl has changed to %s! Using instance-local URL.",
                self._api_url_b2b,
            )
    ...
    def _normalized_base_url(base_url: str | None) -> str | None:
        """Normalize base URLs for robust scope matching."""
        if base_url is None:
            return None
    ...
    def _resolve_api_base_url(
        ...
        """Resolve configured API URL and identify gateway token scope."""
        normalized_requested = self._normalized_base_url(base_url)
        normalized_b2c = {
            self._normalized_base_url(const.API_URL),
            self._normalized_base_url(self._api_url),
        }
        normalized_b2b = {
            self._normalized_base_url(const.API_URL_B2B),
            self._normalized_base_url(self._api_url_b2b),
        }

        if normalized_requested is None or normalized_requested in normalized_b2c:
            return self._api_url, "b2c"
        if normalized_requested in normalized_b2b:
            return self._api_url_b2b, "b2b"
    ...
    def _call_api(
        ...
        resolved_base_url, api_scope = self._resolve_api_base_url(base_url)
        url = parse.urljoin(resolved_base_url, endpoint)
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            if (
                ...
                try:
                    ...
                    time.sleep(random.uniform(0.05, 0.2) * attempt)
                    continue
                except SmartmeterError as exception:
                    logger.warning(
                        "Failed to refresh API gateway keys after status %s for endpoint '%s': %s",
