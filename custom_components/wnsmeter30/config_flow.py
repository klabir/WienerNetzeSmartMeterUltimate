...
def _map_auth_exception(exception: Exception) -> str:
    """Map Smartmeter auth exceptions to config-flow error keys."""
    if isinstance(exception, SmartmeterLoginError):
        return "auth"
    if isinstance(exception, (SmartmeterConnectionError, SmartmeterQueryError)):
    ...
    async def async_step_user(self, user_input: Optional[dict[str, Any]] = None):
        ...
        if user_input is not None:
            try:
                ...
                errors["base"] = _map_auth_exception(exception)
                _LOGGER.warning(
                    "Wiener Netze auth validation failed with %s",
                    exception.__class__.__name__,
                )
                _LOGGER.debug("Auth validation traceback", exc_info=exception)
            if not errors:
