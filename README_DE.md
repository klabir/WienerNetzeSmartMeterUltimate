...
Wenn `Use alias also for statistic and entity IDs` aktiviert ist und ein Alias existiert, verwenden Entitaets-IDs einen Alias-basierten Slug anstelle von `<zaehlpunkt>`.
...
- `Days retreived by historical API Calls`: `1095` (erlaubter Bereich `1-3650`)
...
- `Enable raw Api Response written to /config/tmp/wnsm_api_calls`: `False` (verwendet den ersten beschreibbaren Pfad aus `/config/tmp/wnsm_api_calls`, `/homeassistant/tmp/wnsm_api_calls`, `/tmp/wnsm_api_calls`)

Hinweis zur Raw-API-Protokollierung:
- Wenn Raw-API-Logging aktiviert ist, werden vorhandene Dateien und Ordner unter dem gewaehlten Log-Stamm bei der Vorbereitung des Log-Verzeichnisses bereinigt.
...
### Gesamtverbrauch gestern aus historischer Tagessumme - Tile
