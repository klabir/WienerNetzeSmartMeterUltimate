...
| `wnsmeter30:<zaehlpunkt_lowercase>_meter_read` | Enabled | Toggle: `Enable total consumption historical values (Zählerstand) and statistics (Suffix _meter_read).` Energy Dashboard stream (`sum` is monotonic). |
...
- `Enable total consumption historical values (Zählerstand) and statistics (Suffix _meter_read).`: `True`
