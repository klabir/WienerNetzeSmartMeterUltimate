...
def test_warning_b2c_api_key_change(requests_mock,caplog):
    ...
    sm = smartmeter().login()
    assert sm._api_url == "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2C/2.0"
    assert const.API_URL == "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2C/1.0"
...
def test_warning_b2b_api_key_change(requests_mock,caplog):
    ...
    sm = smartmeter().login()
    assert sm._api_url_b2b == "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2B/2.0"
    assert const.API_URL_B2B == "https://api.wstw.at/gateway/WN_SMART_METER_PORTAL_API_B2B/1.0"
    assert 'The b2bApiUrl has changed' in caplog.text


def test_resolve_api_base_url_accepts_trailing_slash_for_b2c():
    sm = smartmeter()

    resolved_url, api_scope = sm._resolve_api_base_url(f"{const.API_URL}/")

    assert api_scope == "b2c"
    assert resolved_url == sm._api_url


def test_resolve_api_base_url_accepts_trailing_slash_for_b2b():
    sm = smartmeter()

    resolved_url, api_scope = sm._resolve_api_base_url(f"{const.API_URL_B2B}/")

    assert api_scope == "b2b"
    assert resolved_url == sm._api_url_b2b


def test_resolve_api_base_url_keeps_unknown_scope():
    sm = smartmeter()
    unknown_url = "https://example.invalid/api"

    resolved_url, api_scope = sm._resolve_api_base_url(unknown_url)

    assert api_scope is None
    assert resolved_url == unknown_url
...
@pytest.mark.usefixtures("requests_mock")
def test_call_api_401_refresh_failure_is_raised_and_logged(requests_mock, monkeypatch, caplog):
    expect_login(requests_mock)
    requests_mock.get(
        re.compile(r".*/zaehlpunkte$"),
        status_code=401,
        json={"error": "unauthorized"},
    )
    sm = smartmeter().login()

    def _raise_refresh_failure(_token):
        raise SmartmeterConnectionError("forced refresh failure")

    monkeypatch.setattr(sm, "_get_api_key", _raise_refresh_failure)

    with pytest.raises(SmartmeterConnectionError, match="forced refresh failure"):
        sm.zaehlpunkte()
    assert "Failed to refresh API gateway keys after status 401" in caplog.text
...
@pytest.mark.usefixtures("requests_mock")
def test_historical_meter_reading_prefers_consumption_obis(requests_mock: Mocker):
    z = zaehlpunkt_response([enabled(zaehlpunkt())])[0]
    zp = z["zaehlpunkte"][0]["zaehlpunktnummer"]
    customer_id = z["geschaeftspartner"]
    expect_login(requests_mock)
    ...
    hist = smartmeter().login().historical_meter_reading()

    assert hist["obisCode"] == "1-1:1.8.0"
    assert requests_mock.request_history[-1].qs["wertetyp"][0].upper() == "METER_READ"


@pytest.mark.usefixtures("requests_mock")
def test_historical_meter_reading_prefers_feeding_obis(requests_mock: Mocker):
    z = zaehlpunkt_response([enabled(zaehlpunkt_feeding())])[0]
    zp = z["zaehlpunkte"][0]["zaehlpunktnummer"]
    customer_id = z["geschaeftspartner"]
    expect_login(requests_mock)
    ...
    assert hist["obisCode"] == "1-1:2.8.0"
    assert requests_mock.request_history[-1].qs["wertetyp"][0].upper() == "METER_READ"
